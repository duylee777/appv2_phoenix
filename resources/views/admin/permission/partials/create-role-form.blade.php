            ...
            <form data-route="{{ route('role.store') }}" method="POST" class="p-4 md:p-5">
                ...
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Vai trò</label>
                        <input type="text" name="name" id="new-name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Nhập vai trò ..." required="">
                    </div>
                    <div class="col-span-2">
                        ...
                        <ul class="grid grid-cols-2 gap-x-2 gap-y-2 w-full text-sm font-medium text-gray-900 bg-white">
                            ...
                            <li class="w-full border border-gray-200 rounded-lg">
                                <div class="flex items-center ps-3">
                                    <input id="permission-{{$permission->id}}" type="checkbox" name="permissions" value="{{ $permission->name }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">
                                    <label for="permission-{{$permission->id}}" class="w-full py-3 ms-2 text-sm font-medium text-gray-900">{{ $permission->name }}</label>
                                </div>
                            </li>
                            @endforeach
                        </ul>
